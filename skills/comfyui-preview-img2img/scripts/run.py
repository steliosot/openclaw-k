#!/usr/bin/env python3
"""Fast 12-step preview of img2img transformation using ComfyUI."""
...
def build_workflow(image_name, prompt, negative_prompt, steps, denoise, cfg, seed):
    return {
        ...
        "8": {
            ...
            "inputs": {"images": ["7", 0], "filename_prefix": "openclaw_preview_remix"},
        },
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Fast 12-step preview of img2img transformation"
    )
    parser.add_argument("input", help="Input image path")
    parser.add_argument("output", help="Output file path")
    parser.add_argument("--prompt", required=True, help="Style/transformation prompt")
    parser.add_argument("--negative",
                        default="watermark, text, blurry, low quality, artifacts")
    ...
    print(f"Generating preview img2img (fast 12-step)...")
    print(f"  Input: {args.input}")
    print(f"  Style: \"{args.prompt}\"")
    print(f"  Denoise: {args.denoise}, Steps: {args.steps}, CFG: {args.cfg}, Seed: {seed}")

    server_name = upload_image(args.input)
    workflow = build_workflow(
        server_name, args.prompt, args.negative,
        args.steps, args.denoise, args.cfg, seed,
    )
